...
def gen_include_inversemodhuff():
  print("#include \"inversemod_bls12381.huff\"")
...
p_g2_1 = p_g1_1 + SIZE_G1
p_g1_2 = p_g2_1 + SIZE_G2 
p_g2_2 = p_g1_2 + SIZE_G1

def gen_deserialize_miller_loop_input():
    calldata_offset = 0
...
def gen_miller_loop_test():
    ...
    gen_consts(1)
    gen_deserialize_miller_loop_input()
    gen_miller_loop(buffer_miller_output,buffer_inputs,buffer_inputs+96,mod)
...
def main():
    if len(sys.argv) != 2 or (sys.argv[1] != 'miller_loop' and sys.argv[1] != 'final_exp'):
        raise Exception("input should be 'miller_loop' or 'final_exp'")

    if sys.argv[1] == 'miller_loop':
        gen_miller_loop_test()
    elif sys.argv[1] == 'final_exp':
        gen_final_exp_test()
